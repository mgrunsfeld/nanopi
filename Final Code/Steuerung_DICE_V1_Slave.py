...
#Seems like only 1/32 step size is used so I wired it accordingly and removed this. Wire and declare pin M1 and M0 to change step size
...
if (SMC == 1):
    ...
    while (checkTime < (SMC_timeMax + 1) and P < 1400):
		...
		length = ser.read(1)  #read the length of alpha
		ser.write("recieved")
		time.sleep(.05)
		alpha = ser.read(length) #read alpha from the master cell
