...
#set gpio numbers to raw linux numbering scheme (see nanopi friendlyarm wiki for pinout)
GPIO.setmode(GPIO.RAW)
GPIO.setwarnings(False)


#initializing variables, setting defaults, defining pins, defining i2c bus
#program parameters
...
#Motor Controller 
#M1 = ??? 			#driven high by 3.3v directly
#M0 = ??? 			#left floating for 1/32 step size
DIR = 3
SLEEP = 0

#PWM for STEP pin of motor driver 
STEP = 2
freq = 240000
dc = 50
...
stepTime      = (.0000208333)    #time the motor has for a step in [µs]
...
#Set values to default which will stay unless it's changed
...
def calibratePressure():
	...
	global C5
	global C6
	# MS5803_01BA address, 0x77(118) (if chip select is high, it will be 0x76)
...
#(Taken almost directly from Steuerung_DICE.ino)
def step(alpha,angle,stepCounter):
	...
	dalpha = alpha - angle
	requiredSteps = abs((dalpha) * iplanet / anglePerStep)
	localStepCounter = 0
	accelerationSteps = 15 * stepMode
	loopCheck = time.time() #redefining the start of the loop for elspased time
	looptime = (loopCheck - loopTimeStart)
	GPIO.output(SLEEP, 1)
	time.sleep(.000001)
	while (looptime < (iterationtime - .001)):
		if (requiredSteps > 1):
			if (dalpha > 0):
				GPIO.output(DIR, 1)
				time.sleep(.0000002)
				while ((localStepCounter <= requiredSteps) and (looptime < (iterationtime - .001)) and  angle < alpha):
					motor_step.start(dc)
					time.sleep(0.00000208333)  #1 full cycle: (1/240000 Hz)/2 (/2 b/c duty cycle of 50% means its only high for half a frequency) 
					motor_step.stop()
					stepCounter += 1
					localStepCounter+= 1
					angle = ((stepCounter) * (anglePerStep)) / (iplanet)
				...
				GPIO.output(DIR, 0)
				time.sleep(.0000002)
				while ((localStepCounter <= requiredSteps) and (looptime < (iterationtime - .001)) and angle >= 0 and angle > alpha):
					motor_step.start(dc)
					time.sleep(0.00000208333)  #1 full cycle: (1/240000 Hz)/2 (/2 b/c duty cycle of 50% means its only high for half a frequency) 
					motor_step.stop()
					stepCounter -= 1
	...
	while resetOk != 1:
		...
		if GPIO.input(Switch_Pin) == True:
			...
			while stepCounter < 0:
				...
				time.sleep(.0000002)
				motor_step.start(dc)
				time.sleep(0.00000208333)  #1 full step: (1/240000 Hz)*2
				motor_step.stop()
				stepCounter += 1
			...
			while abs(angle - alpha) > 0.08:
				...
				loopTimeStart = time.time() #redefining the start of the loop for elspased time
...
def sat(x, gamma):
	y = max(min(1.0, x / gamma), -1.0)
	return y

# Estimate ADA depth
def get_xhat1( x1,  n,  rho,  phi,  tau):
  xhat1 = xhat1_prev[n] - ((iterationtime)) * rho * sat(xhat1_prev[n] - x1, phi);
  xhat1_prev[n] = xhat1
  return xhat1
...
def get_xhat2(x1,n,rho,phi,tau):
  ...
  xhat2 = xhat2_prev[n] + ((iterationtime) / (tau)) * (-xhat2_prev[n] - rho * sat(xhat1 - x1, phi))
  xhat2_prev[n] = xhat2
  return xhat2
...
def slidingModeControler(z, zd,z_prev,angle,LAMBDA, eta,phi,a):
	m_c = 0.3793 #mass in kg  (!!!!!!!!!!!!!!!!!NEED TO UPDATE AFTER REBUILD!!!!!!!!!!!!!!)
...
#GPIO SETUP
#GPIO.setup(M0,GPIO.OUT)    !!change wiring setup to put m1 and m0 on GPIO pins!
...
#calibrate the pressure sensor for later use
calibratePressure()

# only 1/32 step size is used so I wired it accordingly and removed this. See manual for more insturtcions about how to implement multiple step sizes
...
#Menu that allows user to select a function or change variable values
while True:
	...
	if response == 1:
		...
		zdesired_set = input("what is the target depth")
		print("target depth =")
		print(zdesired_set)
	elif response == 5:
...
#if selected, run the sliding mode controller
if (SMC == 1):
    ...
    overallTime = 0;
    loopTimeStart = time.time() #redefining the start of the loop for elspased time
	startTime = time.time() #defining the start of the program for elspased time
    ...
    while (checkTime < (SMC_timeMax + 1) and P < 1400):
		...
		z[2] = get_xhat2(z[1], 1, SMO_rho, SMO_phi, SMO_tau)
		if(SinWave == 1):
			elapsed = time.time() #redefining time for elspased time
		...
		if(StepWave == 1):
			functionTimeCheck = time.time() #elapsed time check 
			...
			if(functionTime/1000 >= 2*StepTime):
			   functionTime = time.time() #redefining the start of the function for elspased time
		...
		length = len(str(alpha))
		ser.write(length)
		time.sleep(.05)
		recieved = ser.read(8)
		if recieved == "recieved":
			ser.write(alpha)  #sends alpha to the slave cells if they recieved the length of alpha
		time.sleep(.05)
		angle = step(alpha, angle, stepCounter)
		loopTimeStart = time.time() #redefining the start of the loop for elspased time
		batteryloop += 1
		saveLoop+= 1
		timeStep+= 1
		check = time.time() #redefining the start of the time check for elspased time
		checkTime = check - startTime
		storeValueCounter += 1
		if storeValueCounter == storeValue:
			data = [timestep,z[0],z[1],z[2],angle,alpha,zd[0],zd[1]]
			with open('data.csv','w') as csvFile:
				writer = csv.writer(csvFile)
				writer = write.writerow(data)
			csvFile.close()
			storeValueCounter = 0
    #end programm
    emerge(angle, stepCounter)
...
#if selected, motor is driven to desired position
if (MotorPosition == 1) :
    GPIO.output(SLEEP,1)
    alpha = alphaDesired
    print("alpha = ")
...
#if selected, close the cylinder
if (closeCylinder == 1):
	time.sleep(1)
